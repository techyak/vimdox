...
void _kbfun_press_release(bool press, uint8_t keycode) {
	...
	switch (keycode) {
		case KEY_LeftControl:  (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<0))
				       : (keyboard_modifier_keys_temp&= ~(1<<0));
				       return;
		case KEY_LeftShift:    (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<1))
				       : (keyboard_modifier_keys_temp &= ~(1<<1));
				       return;
		case KEY_LeftAlt:      (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<2))
				       : (keyboard_modifier_keys_temp &= ~(1<<2));
				       return;
		case KEY_LeftGUI:      (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<3))
				       : (keyboard_modifier_keys_temp &= ~(1<<3));
				       return;
		case KEY_RightControl: (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<4))
				       : (keyboard_modifier_keys_temp &= ~(1<<4));
				       return;
		case KEY_RightShift:   (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<5))
				       : (keyboard_modifier_keys_temp &= ~(1<<5));
				       return;
		case KEY_RightAlt:     (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<6))
				       : (keyboard_modifier_keys_temp &= ~(1<<6));
				       return;
		case KEY_RightGUI:     (press)
				       ? (keyboard_modifier_keys_temp |=  (1<<7))
				       : (keyboard_modifier_keys_temp &= ~(1<<7));
				       return;
	}
	...
	for (uint8_t i=0; i<6; i++) {
		if (press) {
			if (keyboard_keys_temp[i] == 0) {
				keyboard_keys_temp[i] = keycode;
				return;
			}
		} else {
			if (keyboard_keys_temp[i] == keycode) {
...
bool _kbfun_is_pressed(uint8_t keycode) {
	// modifier keys
	switch (keycode) {
		case KEY_LeftControl:  if (keyboard_modifier_keys_temp& (1<<0))
					       return true;
		case KEY_LeftShift:    if (keyboard_modifier_keys_temp& (1<<1))
					       return true;
		case KEY_LeftAlt:      if (keyboard_modifier_keys_temp& (1<<2))
					       return true;
		case KEY_LeftGUI:      if (keyboard_modifier_keys_temp& (1<<3))
					       return true;
		case KEY_RightControl: if (keyboard_modifier_keys_temp& (1<<4))
					       return true;
		case KEY_RightShift:   if (keyboard_modifier_keys_temp& (1<<5))
					       return true;
		case KEY_RightAlt:     if (keyboard_modifier_keys_temp& (1<<6))
					       return true;
		case KEY_RightGUI:     if (keyboard_modifier_keys_temp& (1<<7))
					       return true;
	}
